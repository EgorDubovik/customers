...
jQuery(document).ready(function($){
      ...
      for (i=0; i<n; i++){
        ...
        if (day === false){
          ...
          this.drawEventsOrDaynotes(
            ...
            e.content,
            e.category,
            this.getEventColorByCategory(e.category),
    ...
    Calendar.prototype.getCategories = function(object, attribute1, attribute2){
      ...
      for (var i=0; i<n; i++){
        ...
        if ( parseInt(e[attribute1]) >= fromTimeStamp && parseInt(e[attribute2]) <= toTimeStamp ){
          categories.push({"category": e.category, "color": e.color});
        }
      }
    ...
    Calendar.prototype.determineEventsCategories = function() {
        this.eventCategoryColor = this.getCategories(this.events, 'start', 'end');
    };
  
    Calendar.prototype.getEventColorByCategory = function(category) {
        for(var i = 0;i < this.eventCategoryColor.length;i++)
            if(this.eventCategoryColor[i].category == category) 
                return this.eventCategoryColor[i].color;
